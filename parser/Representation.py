...
# TODO.
#--------------------------------------------------------------------------------------
class Text2TFIDF_Salton88(Text2TF):
    def __init__(self, grouper, l_vocabulary=None, rootPath="", pClass=None, corpusReader=PlaintextCorpusReader):
        super(Text2TFIDF_Salton88, self).__init__(grouper, l_vocabulary, rootPath, pClass, corpusReader)

        D = len(self.df_data)
        for wordColumn in self.df_data:
            try:
                DFti = len(self.df_data[self.df_data[wordColumn] > 0])
                idf = np.log((D + 1)/(DFti + 1))
                self.df_data[wordColumn] = self.df_data[wordColumn].map(lambda x: np.log(1 + x) * idf)
            except:
                print("ERROR* D=%d; DFti=%d; idf=%.2f; wordColumn=%s" %(D, DFti, idf, wordColumn))
...
# TODO.
#--------------------------------------------------------------------------------------
class Text2TFIDFW2(Text2TF):
    def __init__(self, grouper, l_vocabulary=None, rootPath="", N=1, pClass=None, corpusReader=PlaintextCorpusReader):
        super(Text2TFIDFW2, self).__init__(grouper=grouper, l_vocabulary=l_vocabulary, rootPath=rootPath, 
                    pClass=pClass, curpusReader=corpusReader)
        ...
        self.df_data.fillna(0, inplace=True)
        ma = self.df_data.rolling(window=N).apply(lambda x: (x * factors).sum())
        ma = ma.dropna(axis=0, how="all").copy()

        #Evaluating TF-IDF
        D = len(self.df_data)
        for wordColumn in ma:
            try:
                DFti = len(self.df_data[self.df_data[wordColumn] > 0])
                idf = np.log((D + 1)/(DFti + 1))

                self.df_data[wordColumn] = ma[wordColumn].map(lambda x: np.log(1 + x) * idf)
            except:
                print("ERROR* D=%d; DFti=%d; idf=%.2f; wordColumn=%s" %(D, DFti, idf, wordColumn))
